...
| `maintenance/printers/Setup-Printer.ps1` | Generic HP Universal Print Driver (PCL6) network printer setup, parameterized by printer name/IP; per-customer `*_wrappers/` folders hold the thin wrapper scripts that call it (e.g. `vsgn_wrappers/syncro_wrapper_vsgn_d11_32.ps1` for VSGN's "D11-32 Container MFP M430f"; `hux_wrappers/` for HUX's "HP E78630z Andelfingen", incl. a second "Briefpapier" (letterhead) queue sharing the same port) |
...
| `maintenance/Debug-CalendarPermission.ps1` | Diagnostic (+ opt-in `-Remediate` ACE rewrite, `-CheckCorruption` mailbox repair scan) for "Owner on Get-MailboxFolderPermission but Outlook still blocks calendar item creation" (OWA works fine, Outlook crashes in EMSMDB32.DLL opening the mailbox) — not a permissions problem, points at store/folder corruption; `maintenance/syncro_wrapper_debug_calendar_permission.ps1` is the thin wrapper to paste into SyncroMSP, scoped to the Exchange server asset, Run As logged-in user |
...
Generic engine that sets up a network printer using the HP Universal Print Driver (PCL6). Not a direct SyncroMSP endpoint script — run indirectly via a thin per-printer `syncro_wrapper_*.ps1` (see below), which is what actually gets pasted into Syncro. Takes `-PrinterName` and `-PrinterIP` (plus optional `-PortName`/`-DriverUrl`/`-ColorMode`), so the same driver-install logic is shared across printers/customers instead of duplicated per wrapper.
...
Removes any existing printer with the same `-PrinterName` first (idempotent) — matched by name only, not port, so a second queue sharing the same physical printer's port (e.g. a "letterhead" queue for a printer that already has a plain-paper queue set up) isn't torn down by this run. Creates the port and printer via `Add-PrinterPort`/`Add-Printer` (`Add-PrinterPort` skips creation if the port already exists, for the same shared-port case), sets the default color mode via `Set-PrintConfiguration -ColorMode` (default `Color`, pass `-ColorMode Monochrome` for B/W-only queues), then cleans up the extracted files and the zip afterward.

**Two queues, one physical printer:** to set up e.g. a "letterhead" queue (first page from the letterhead tray, remaining pages from the normal tray) alongside a plain-paper queue on the same device, run the script twice with different `-PrinterName` values but the same `-PrinterIP`/`-PortName`. The per-tray "different first page" behavior itself is an HP-driver-specific setting not exposed by `Set-PrintConfiguration` — configure it once manually in the queue's printing preferences (Paper/Quality tab), then replicate via `rundll32 printui.dll,PrintUIEntry /Ss /n "<queue>" /a export.dat g d` (export) and `/Sr` (import) if it needs to be scripted onto other machines.

**Per-printer wrappers**, grouped in a `<customer>_wrappers/` folder each, are the files to copy into SyncroMSP:
- `vsgn_wrappers/syncro_wrapper_vsgn_d11_32.ps1` — "D11-32 Container MFP M430f" at customer VSGN, IP `192.168.0.32`.
- `hux_wrappers/wrapper_hux_E78630z_andelfingen.ps1` — "HP E78630z Andelfingen" at customer HUX, IP `192.168.40.32`, plain-paper queue.
- `hux_wrappers/wrapper_hux_E78630z_andelfingen_briefpapier.ps1` — same HUX printer/port, second "Briefpapier" queue (`-ColorMode Monochrome`); the letterhead-tray-for-first-page behavior itself still needs the one-time manual driver configuration described above.

Each wrapper is a few lines: it downloads and runs the current `Setup-Printer.ps1` from this repo with that printer's fixed `-PrinterName`/`-PrinterIP`, so fixes to the shared driver logic take effect everywhere without editing wrappers again. No Required File attachment is needed. To add another printer, copy an existing wrapper and change the name/IP; to add a second queue for an existing printer, copy the relevant wrapper into the same customer folder, change `-PrinterName` and add `-PortName` pointing at the existing queue's port.
...
### `maintenance/Debug-CalendarPermission.ps1`

Diagnostic (mostly read-only, two opt-in actions) for a specific mismatch on shared/room mailboxes: `Get-MailboxFolderPermission` already shows a user as `Owner` on the mailbox's calendar folder, but Outlook still throws "You don't have permission to create an entry in this folder" while OWA works fine for the same user. Ruled out in the field for the originating ticket, in order: an Outlook delegate assignment as the source of the "Owner" entry (no `SharingPermissionFlags=Delegate`), a stale client-side Outlook cache (reproduced on a brand new client/profile), a stale ACE fixable by remove+re-add (`-Remediate`, tried, no change), missing mailbox-level rights (granting `FullAccess` didn't fix it either), and it being user-specific (a second user with their own independent `Owner` ACE hit the identical error). Confirmed instead: adding the mailbox as a full additional account in Outlook reliably crashes Outlook with an access violation inside `EMSMDB32.DLL` (Outlook's on-prem Exchange MAPI provider) — not used by OWA/EWS, consistent with OWA working throughout, and pointing at store/folder-level corruption rather than a permissions problem. Remaining escalation: broader `New-MailboxRepairRequest` corruption types via `-CheckCorruption` (below), then a stale/duplicate raw MAPI ACL entry invisible to `Get-`/`Remove-MailboxFolderPermission` — a [documented Exchange pattern](https://blog.icewolf.ch/archive/2022/12/30/how-to-delete-mapi-permission-if-remove-mailboxfolderpermission-does-not/) needing MFCMAPI (`Other Tables` → `ACL Table` on the folder) to find and remove directly.

Not run directly in Syncro — `syncro_wrapper_debug_calendar_permission.ps1` is the thin wrapper to paste into **Scripting → Scripts** (same download-from-GitHub-at-runtime pattern as `syncro_wrapper_debug_outlook_auth.ps1`), scoped to the **Exchange server asset itself**, not a client endpoint. Takes `-MailboxIdentity` (e.g. `siziklein`), `-DelegateIdentity` (e.g. `christa.stocker`), `-FolderName` (default `Calendar`), `-AuditLogDays` (default 90), `-Remediate` (switch, default off), `-CheckCorruption` (switch, default off).

`-CheckCorruption` runs `New-MailboxRepairRequest -DetectOnly` across `ProvisionedFolder`/`SearchFolder`/`MissingSpecialFolders`/`ReplState`/`RestrictionFolder` (batched), then `FolderView` and `AggregateCounts` separately (Microsoft requires these to run alone), polling each job to completion before starting the next — always detect-only, never repairs automatically; it prints the exact repair command if `CorruptionsDetected` is nonzero for any type. Takes several minutes total, so leave it off for routine runs.

Must run with Syncro's **Run As** set to the logged-in user, not System — the `RemoteExchange.ps1` bootstrap (loads Exchange cmdlets into the plain PowerShell host Syncro launches, same bootstrap the EMS shortcut itself runs) opens a WinRM session to the server's own hostname, and as SYSTEM that's a same-machine NTLM authentication attempt, which Windows blocks by default (10-minute hang, then fails). An interactive logged-in account uses Kerberos instead and isn't subject to that block — it also needs to actually hold an Exchange RBAC role (e.g. Organization Management) for the connection and for `Search-AdminAuditLog` to succeed; SYSTEM's computer account normally holds neither.

Checks, in order: (1) unfiltered `Get-MailboxPermission` on the mailbox — confirms whether the user has mailbox-level Full Access at all; (2) `Get-MailboxFolderPermission` on the target folder, specifically the `SharingPermissionFlags` field — a value of `Delegate` there confirms the permission was set via Outlook's own delegate mechanism rather than an admin; identity matching against the delegate resolves through `Get-Recipient` first (DisplayName/Alias/Name/PrimarySmtpAddress), since the cmdlets report the resolved DisplayName, not whatever identity string was passed in; (3) `Search-AdminAuditLog` for `Add-/Set-/Remove-MailboxFolderPermission` calls against the mailbox in the lookback window, to show who/when set it. With `-Remediate`, and only if step (2) found a plain (non-Delegate), valid folder ACE for the delegate, it removes and re-adds that exact same permission to force Exchange to rewrite the ACE cleanly — the standard first attempt before escalating to MFCMAPI. No `Mock-SyncroModule`/`Test-Local.ps1` harness was built for it (see `syncro-new-script` skill guidance) — the remove/re-add is trivially reversible, so the first real Syncro run is the test.

---

### `maintenance/temp-ssh-access/Enable-TempSSHAccess.ps1` / `maintenance/temp-ssh-access/Disable-TempSSHAccess.ps1`
...
4. Deploys a local copy of the disable logic to `C:\ProgramData\ITFactory\TempSSHAccess\Disable-TempSSHAccess.ps1` and registers a one-time Scheduled Task (`ITFactory-TempSSHAccess-AutoDisable`, runs as SYSTEM) that calls it after `-DurationMinutes` — this runs entirely locally, so the auto-revert still fires even if the Syncro connection drops or the machine briefly sleeps (`-StartWhenAvailable`).
5. Optionally writes the expiry timestamp to the `Temp SSH Access Expires` custom asset field (create it first under **Admin → Custom Asset Fields**, type Text) so the current session's expiry is visible directly on the asset in Syncro.
6. Optional `-SshPublicKey "ssh-ed25519 AAAA... comment"` deploys that key for password-less login, in addition to the existing password login (untouched). Written to `C:\ProgramData\ssh\administrators_authorized_keys` — the special path Windows OpenSSH requires for logins by members of the local Administrators group (a regular user's `~\.ssh\authorized_keys` is ignored for admin accounts) — with its ACL locked down to SYSTEM + Administrators only (`icacls /inheritance:r`), since sshd otherwise refuses the file with "bad ownership or modes". Only the key(s) this script itself added are tracked in `state.json` and removed again on disable; any keys already present in that file beforehand are left alone.

Runs entirely invisible to the logged-in user: like all SyncroMSP scripts this executes as SYSTEM in session 0 (no window, no toast), and the scheduled auto-disable task runs `-WindowStyle Hidden` and is registered as a hidden task. `Rmm-Alert` (category `Temp SSH Access`) only fires on failure to enable/disable — routine success is intentionally silent, since a failed *disable* (access left open past its expiry) is the case worth being paged for, not a routine enable/disable.
...
**`maintenance/temp-ssh-access/Disable-TempSSHAccess.ps1`** reverts everything immediately (stops `sshd`, restores its original startup type, disables the firewall rule again, removes any SSH key(s) it added, removes the scheduled task) — run it from Syncro to end a session early instead of waiting for the timer. Safe to run even with no session active. Its logic is duplicated as an embedded here-string inside `Enable-TempSSHAccess.ps1` (needed so the scheduled task can revert locally without depending on Syncro/network reachability at fire time) — keep both in sync if you change the revert logic.

**Local testing without Syncro** — `Enable-TempSSHAccess.ps1`/`Disable-TempSSHAccess.ps1` call `Import-Module $env:SyncroModule` and then `Rmm-Alert`/`Set-Asset-Field`, both of which only exist once that module (injected by the Syncro agent at runtime) is loaded; running the scripts directly on a machine without Syncro fails on those calls. Two dev-only helper files, not deployed to Syncro, work around that:
- **`maintenance/temp-ssh-access/Mock-SyncroModule.psm1`** — a stand-in module providing no-op `Rmm-Alert`/`Set-Asset-Field` implementations that just print to the console instead of talking to Syncro.
- **`maintenance/temp-ssh-access/Test-Local.ps1`** — sets `$env:SyncroModule` to that mock and runs `Enable-TempSSHAccess.ps1` or `Disable-TempSSHAccess.ps1`:
  ```powershell
  powershell.exe -ExecutionPolicy Bypass -File .\Test-Local.ps1 -Action Enable
  powershell.exe -ExecutionPolicy Bypass -File .\Test-Local.ps1 -Action Enable -DurationMinutes 15 -SshPublicKey "ssh-ed25519 AAAA... me@mac"
  powershell.exe -ExecutionPolicy Bypass -File .\Test-Local.ps1 -Action Disable
  ```
  Run elevated (as Administrator) since the underlying scripts manage services, firewall rules, and scheduled tasks.
