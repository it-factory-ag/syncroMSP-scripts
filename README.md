...
| `maintenance/Debug-CalendarPermission.ps1` | Read-only diagnostic for "Owner on Get-MailboxFolderPermission but Outlook still blocks calendar item creation" (OWA works fine): checks unfiltered Get-MailboxPermission, folder-level SharingPermissionFlags (detects Outlook delegate assignments vs. plain ACL grants), and Search-AdminAuditLog for who/when set the permission; `maintenance/syncro_wrapper_debug_calendar_permission.ps1` is the thin wrapper to paste into SyncroMSP, scoped to the Exchange server asset |
...
Read-only diagnostic for a specific mismatch on shared/room mailboxes: `Get-MailboxFolderPermission` already shows a user as `Owner` on the mailbox's calendar folder, but Outlook still throws "You don't have permission to create an entry in this folder" while OWA works fine for the same user. Server-side ACL says the permission is there, so this isn't a missing-grant problem — it's either a stale client-side Outlook permission cache, or the "Owner" entry was actually created as an Outlook delegate assignment (Account Settings → Delegate Access) rather than an admin ACL grant, which behaves differently for calendar item creation.

Not run directly in Syncro — `syncro_wrapper_debug_calendar_permission.ps1` is the thin wrapper to paste into **Scripting → Scripts** (same download-from-GitHub-at-runtime pattern as `syncro_wrapper_debug_outlook_auth.ps1`), scoped to the **Exchange server asset itself**, not a client endpoint. Takes `-MailboxIdentity` (e.g. `siziklein`), `-DelegateIdentity` (e.g. `christa.stocker`), `-FolderName` (default `Calendar`), `-AuditLogDays` (default 90).

The Syncro agent runs scripts as SYSTEM in a plain PowerShell host, which doesn't auto-load Exchange cmdlets the way the Exchange Management Shell shortcut does — the script bootstraps them itself via `RemoteExchange.ps1` (the same bootstrap the EMS shortcut runs, present on every on-prem Exchange 2013+ install). That also means SYSTEM on that server needs an actual Exchange RBAC role assignment for the `Search-AdminAuditLog` step to succeed; a missing role assignment shows up as an `Rmm-Alert` from that step, not a silent failure.

Checks, in order: (1) unfiltered `Get-MailboxPermission` on the mailbox — confirms whether the user has mailbox-level Full Access at all; (2) `Get-MailboxFolderPermission` on the target folder, specifically the `SharingPermissionFlags` field — a value of `Delegate` there confirms the permission was set via Outlook's own delegate mechanism rather than an admin, which is the key fact needed to stop chasing a server-ACL cause; (3) `Search-AdminAuditLog` for `Add-/Set-/Remove-MailboxFolderPermission` calls against the mailbox in the lookback window, to show who/when set it (an empty result also supports the Outlook-delegate theory, since delegate assignments don't go through EMS cmdlets). Purely read-only — no `Mock-SyncroModule`/`Test-Local.ps1` harness was built for it (see `syncro-new-script` skill guidance for diagnostics); the first real Syncro run against the Exchange server is the test.
