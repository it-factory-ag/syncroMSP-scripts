...
| `maintenance/Debug-CalendarPermission.ps1` | Diagnostic (+ opt-in `-Remediate` fix) for "Owner on Get-MailboxFolderPermission but Outlook still blocks calendar item creation" (OWA works fine): checks unfiltered Get-MailboxPermission, folder-level SharingPermissionFlags (Outlook delegate vs. plain ACL grant vs. server-side MAPI ACL desync), and Search-AdminAuditLog for who/when set the permission; `maintenance/syncro_wrapper_debug_calendar_permission.ps1` is the thin wrapper to paste into SyncroMSP, scoped to the Exchange server asset, Run As logged-in user |
...
Diagnostic (mostly read-only, one opt-in fix) for a specific mismatch on shared/room mailboxes: `Get-MailboxFolderPermission` already shows a user as `Owner` on the mailbox's calendar folder, but Outlook still throws "You don't have permission to create an entry in this folder" while OWA works fine for the same user. Server-side ACL says the permission is there, so this isn't a missing-grant problem. Ruled out in the field for the originating ticket: an Outlook delegate assignment (Account Settings → Delegate Access) as the source of the "Owner" entry (would show `SharingPermissionFlags=Delegate`; it didn't), and a stale client-side Outlook cache (reproduced on a brand new client/profile). Leading remaining cause: a server-side desync between the "clean" ACE `Get-MailboxFolderPermission` reports (read by OWA/EWS) and a stale/duplicate entry in the folder's raw MAPI ACL table, which Outlook (MAPI) actually evaluates — a [documented Exchange pattern](https://blog.icewolf.ch/archive/2022/12/30/how-to-delete-mapi-permission-if-remove-mailboxfolderpermission-does-not/) where such entries can be invisible to `Get-`/`Remove-MailboxFolderPermission` and need MFCMAPI (`Other Tables` → `ACL Table` on the folder) to find and remove directly.
